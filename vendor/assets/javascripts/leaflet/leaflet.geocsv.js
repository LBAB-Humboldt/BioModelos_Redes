...
L.GeoCSV = L.GeoJSON.extend({
  ...
  _csv2json: function (csv) {
    ...
    for (var num_linea = 0; num_linea < csv.length; num_linea++) {
      ...
      var campos = $.csv.toArray(csv[num_linea].trim())
      //var campos = csv[num_linea].trim().split(regex)
      ...
      if (campos.length==titulos.length && lng<180 && lng>-180 && lat<90 && lat>-90 && campos[14] != 'No') {
        var feature = {};
